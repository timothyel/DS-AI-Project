...
# Load pre-trained BERT model (lightweight) untuk semantic similarity
model_bert = SentenceTransformer("all-MiniLM-L6-v2")

def extract_keywords(text):
    """
    Ambil keyword dari noun chunks dalam teks.
    Cocok untuk mendeteksi skill, tools, atau job function.
    """
    doc = nlp_spacy(text)
...
def split_responsibilities_flexibly(text):
    ...
    - Bullet points
    - Numbered points
    - Kalimat panjang sebagai fallback
    """
...
def compute_similarity(text_a, text_b):
    """
    Hitung similarity antar dua teks (misal: CV vs JD) menggunakan cosine similarity dari BERT.
    """
...
def compute_similarity_per_point(cv_text, jd_text):
    """
    Hitung similarity antara setiap poin tanggung jawab di CV dengan JD.
    """
...
def estimate_years_of_experience(text):
    """
    Estimasi total pengalaman kerja dari tanggal-tanggal dalam CV.
    Format yang dideteksi: (e.g. Jan 2020 - Mar 2023)
    """
    patterns = [
    ...
    return round(total_months / 12, 1)  # dalam tahun

def convert_to_date(month_or_year, year=None):
    """
    Konversi teks ke objek datetime. Dukung format bulan + tahun atau hanya tahun.
    """
    try:
        if year is None:  # format: YYYY
        ...
        try:
            return datetime.strptime(f"{month_or_year} {year}", "%B %Y")  # full month
        except:
            return None

def detect_required_experience(jd_text):
    """
    Deteksi minimum tahun pengalaman dari JD (misal "2+ years experience", "at least 3 years").
    """
    match = re.search(r'(\d{1,2})\s*(\+?\s*years?|tahun)[\w\s]*(experience|pengalaman)?', jd_text, re.IGNORECASE)
    if match:
